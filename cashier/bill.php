...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Check if cart items are passed directly
    if (isset($_POST['cart_items'])) {
        $cart_items = json_decode($_POST['cart_items'], true);
    } else {
        // Fallback: Get items from database
        $items_sql = "SELECT 
                        product_id,
                        product_name,
                        product_color,
                        ...
                        discount_input
                      FROM bill_items 
                      WHERE bill_no = '$bill_no' AND return_status = 'sold'";
    ...
    // Get cashier name
    $cashier_sql = "SELECT name FROM cashier_users WHERE cashier_id = '$cashier_id'";
...
<body>
    <div class="receipt-container">
        ...
        <div class="header-section">
            <div class="store-name">KIDS BERRY - JAFFNA</div>
            <div class="store-tagline">Kids & Baby Needs</div>
            <div class="store-address"  style="font-weight: bold !important;" >272 Manipay-karainagar Rd</div>
            <div class="store-address" style="font-weight: bold !important;" >Jaffna</div>
            ...
            if (!empty($cart_items)) {
                foreach ($cart_items as $index => $item) {
                    ...
                    if ($item['discount'] > 0) {
                        ...
                        $discount_display = '';
                        $discount_type = isset($item['discount_type']) ? $item['discount_type'] : 'rs';
                        $discount_input = isset($item['discount_input']) ? $item['discount_input'] : $item['discount'];
                ...
                echo '<div style="text-align: center; font-style: italic; font-size: 9px;">No items found</div>';
            }
            ?>
        </div>
