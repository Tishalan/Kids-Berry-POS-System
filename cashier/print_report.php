...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['print_report'])) {
    ...
    if (!empty($cashier_id)) {
        $cashier_sql = "SELECT name FROM cashier_users WHERE cashier_id = ?";
    ...
    if (!empty($logged_cashier_id)) {
        $logged_cashier_sql = "SELECT name FROM cashier_users WHERE cashier_id = ?";
...
<?php
    ...
    <body style="background: #666;">
        <div class="message-box">
            ...
            <a href="report.php" class="btn">Go to Reports Page</a>
        </div>
    </body>
    </html>';
}
