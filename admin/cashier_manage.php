...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['add_cashier'])) {
        ...
        $table_name = ($branch == 'branch1') ? 'cashier_users' : 'cashier_users2';
    ...
    if (isset($_POST['update_cashier'])) {
        ...
        $check_sql = "SELECT * FROM $table_name WHERE email = ? AND cashier_id != ?";
        $check_stmt = $conn->prepare($check_sql);
        $check_stmt->bind_param("si", $email, $cashier_id);
        $check_stmt->execute();
        $result = $check_stmt->get_result();
        ...
        if ($result->num_rows > 0) {
            ...
            if (!empty($_POST['password'])) {
                $password = password_hash($_POST['password'], PASSWORD_DEFAULT);
                $update_sql = "UPDATE $table_name SET name = ?, email = ?, phone_no = ?, address = ?, password = ? WHERE cashier_id = ?";
                $update_stmt = $conn->prepare($update_sql);
                $update_stmt->bind_param("sssssi", $name, $email, $phone_no, $address, $password, $cashier_id);
            } else {
                $update_sql = "UPDATE $table_name SET name = ?, email = ?, phone_no = ?, address = ? WHERE cashier_id = ?";
                $update_stmt = $conn->prepare($update_sql);
                $update_stmt->bind_param("ssssi", $name, $email, $phone_no, $address, $cashier_id);
            }
    ...
    if (isset($_POST['toggle_status'])) {
        ...
        $update_sql = "UPDATE $table_name SET status = ? WHERE cashier_id = ?";
        $update_stmt = $conn->prepare($update_sql);
        $update_stmt->bind_param("si", $new_status, $cashier_id);
    ...
    if (isset($_POST['delete_cashier'])) {
        ...
        $delete_sql = "DELETE FROM $table_name WHERE cashier_id = ?";
        $delete_stmt = $conn->prepare($delete_sql);
        $delete_stmt->bind_param("i", $cashier_id);
    ...
    if (isset($_POST['view_branch'])) {
        ...
        $_SESSION['selected_branch_cashier'] = $selected_branch;
    }
}

// Get selected branch from session or default to branch1
$selected_branch = isset($_SESSION['selected_branch_cashier']) ? $_SESSION['selected_branch_cashier'] : 'branch1';

// Get cashiers from the selected branch table
$table_name = ($selected_branch == 'branch1') ? 'cashier_users' : 'cashier_users2';
...
<head>
    ...
    <title>Kids Berry - Sales Officer Management</title>
...
<body>
    ...
    <div class="container">
        ...
        <div class="sidebar" id="sidebar">
            ...
            <ul class="nav-links">
                ...
                <li><a href="cashier_manage.php" class="active"><i class="fas fa-user-tie"></i> Sales Officer Management</a></li>
        ...
        <div class="main-content" id="mainContent">
            <div class="header">
                <h1><i class="fas fa-user-tie"></i> Sales Officer Management</h1>
                <div class="user-info">
            ...
            <div class="branch-selector">
                ...
                <form method="post" action="" id="branchForm">
                    <div class="branch-radio-group">
                        <label class="branch-option">
                            ...
                            Branch 1 <span class="branch-badge" style="background: linear-gradient(135deg, #6a0dad, #4b0082);">Cashier Users Table</span>
                        </label>
                        <label class="branch-option">
                            <input type="radio" name="selected_branch" value="branch2" <?php echo ($selected_branch == 'branch2') ? 'checked' : ''; ?> onchange="this.form.submit()">
                            Branch 2 <span class="branch-badge" style="background: linear-gradient(135deg, #228b22, #32cd32);">Cashier Users2 Table</span>
                        </label>
                    </div>
                ...
                <div style="margin-left: auto;">
                    <span class="branch-badge">
                        <i class="fas fa-database"></i> Currently viewing: <?php echo ($selected_branch == 'branch1') ? 'Branch 1 (cashier_users)' : 'Branch 2 (cashier_users2)'; ?>
                    </span>
                </div>
            </div>
            ...
            <div class="content-layout">
                ...
                <div class="card">
                    <div class="card-header">
                        <h2 class="card-title"><i class="fas fa-user-plus"></i> Add New Sales Officer</h2>
                    ...
                    <form method="post" action="">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="branch" name="branch" required>
                                <option value="branch1" <?php echo ($selected_branch == 'branch1') ? 'selected' : ''; ?>>Branch 1 (cashier_users table)</option>
                                <option value="branch2" <?php echo ($selected_branch == 'branch2') ? 'selected' : ''; ?>>Branch 2 (cashier_users2 table)</option>
                            </select>
                        </div>
                ...
                <div class="card">
                    <div class="card-header">
                        <h2 class="card-title"><i class="fas fa-list"></i> Sales Officer - <?php echo ($selected_branch == 'branch1') ? 'Branch 1' : 'Branch 2'; ?></h2>
                        <span class="badge"><?php echo $cashiers->num_rows; ?> Sales officers</span>
                    </div>
                                            ...
                                            <td>
                                                <div class="action-buttons">
                                                    <button class="btn btn-primary btn-sm edit-cashier" 
                                                            data-id="<?php echo $cashier['cashier_id']; ?>"
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="edit_branch_select" name="branch" required>
                            <option value="branch1">Branch 1 (cashier_users table)</option>
                            <option value="branch2">Branch 2 (cashier_users2 table)</option>
                        </select>
                    </div>
                </div>
    ...
    <div class="modal" id="deleteModal">
        <div class="modal-content">
            ...
            <form method="post" action="">
                <div class="modal-body">
                    <input type="hidden" name="cashier_id" id="delete_cashier_id">
    ...
    <script>
        ...
        editButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                document.getElementById('edit_cashier_id').value = cashierId;
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                document.getElementById('delete_cashier_id').value = cashierId;
